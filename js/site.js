...
(function(){
  ...
  function applyDir(dir){
    ...
    localStorage.setItem('agro_dir', dir);
  }
  ...
  var TWEAKS = {
    ...
    accent:{label:'Cor de acento', type:'swatch', options:['#2F9E5E','#179E57','#B16A40','#3E6E89','#5C8A3A'], def:'#2F9E5E'},
  ...
  var DIR_ACCENT = {raizes:'#2F9E5E', sistema:'#179E57', terra:'#B16A40'};

  function readTweaks(){
    var s = {}; try{ s = JSON.parse(localStorage.getItem('agro_tweaks')||'{}'); }catch(e){}
    for(var k in TWEAKS){ if(s[k]==null) s[k]=TWEAKS[k].def; }
    return s;
  }
  function writeTweaks(s){ localStorage.setItem('agro_tweaks', JSON.stringify(s)); }
  ...
  function boot(){
    ...
    var lang = localStorage.getItem('agro_lang')||'pt';
    applyLang(lang);
